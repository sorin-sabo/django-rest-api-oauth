...
class TokenAuthentication(BaseAuthentication):
    """
    Token authentication using AWS Cognito token
    ...
    def get_token(request):
        ...
        :return:
            - {String} Request's 'Authorization' Id/Access token - When correct Authorization:' header
            - None - when no 'Authorization:' header or Authorization:' header different then 'Bearer ...'
        ...
        if smart_text(token_type.lower()) != 'bearer':
            return None
    ...
    def authenticate_header(self, request):
        """
        :param request: {Object} Client request - not used but required
        :raises: 401 responses for authentication failures, instead of 403 - method required by the DRF
        Details on https://www.django-rest-framework.org/api-guide/authentication/#custom-authentication.
        """
